...
func _on_item_selected(index: int) -> void:
	...
	var ref: Array = Handle.main_node._parse_entry_ref(item_list.get_item_text(index))
	...
	if Handle.strings.has(item_local_name):
		...
		for string_container: IStringContainer in Handle.strings[item_local_name]:
			...
			Handle.main_node.string_selector.add_item(Handle.main_node._string_progress_prefix(string_container) + str(string_container.layer_strings[0]))
		Handle.main_node.string_selector.select(idx_zero)
		Handle.main_node.string_selector.ensure_current_is_visible()
		for i in range(Handle.main_node.dialogue_selector.get_item_count()):
			...
			if Handle.main_node.dialogue_selector_entry_at(i) == item_local_name:
				Handle.main_node.dialogue_selector.select(i)
